defmodule Membrane.Parent do
  ...
  """
  @callback handle_stopped_to_prepared(context :: any, state :: internal_state_t()) :: callback_return_t
  ...
  """
  @callback handle_playing_to_prepared(context :: any, state :: internal_state_t()) :: callback_return_t

  @doc """
  Callback invoked when bin is in `:playing` state, i.e. all its children
  are in this state.
  """
  @callback handle_prepared_to_playing(context :: any, state :: internal_state_t()) :: callback_return_t

  @doc """
  Callback invoked when bin is in `:playing` state, i.e. all its children
  are in this state.
  """
  @callback handle_prepared_to_stopped(context :: any, state :: internal_state_t()) :: callback_return_t
  ...
  """
  @callback handle_other(message :: any, context :: any,  state :: internal_state_t()) :: callback_return_t
  ...
  defmacro __using__(options) do
    ...
    quote do
      ...
      @impl true
      def handle_spec_started(_new_children, _ctx, state), do: {:ok, state}
      ...
      defoverridable handle_stopped_to_prepared: 2,
